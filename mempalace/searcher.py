...
def _finalize_candidate_hits(
    *,
    ...
    hits = _dedupe_logical_hits(
        _hybrid_rank(hits, query, metric=metric, stop_words=stop_words)
    )[:n_results]
    for h in hits:
...
def _query_drawer_variants(
    ...
    *,
    ...
    ``pool_size`` comes from the upstream candidate-pool sizing (it widens the
    pool while a date window is active). When given, it replaces the local
    heuristic so the variant fan-out and the upstream date filter agree on how
    many candidates to fetch.
    """
    merged = {}
    fetch_limit = pool_size if pool_size > 0 else max(n_results * 5, 25)
...
def search_memories(
    ...
    for stored_drawer_id, doc, meta, dist in zip(
        ...
        entry = {
            ...
            # Physical row id. Since 3.7.1 this comes from _aligned_query_ids,
            # which keeps ids aligned with documents instead of indexing by
            # position (the old local heuristic could drift and synthesise).
            "_row_id": stored_drawer_id,
        }
        if closet_preview:

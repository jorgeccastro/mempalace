...
def tool_diary_delete(entry_id: str):
    ...
    Chunk-safe (v3.4.1): tool_diary_write splits oversized entries into a
    group of ``{entry_id}_chunk_NNNNNN`` rows tagged ``parent_entry_id``
    (no row is stored under the bare ``entry_id`` for the chunked path). A
    naive ``delete(ids=[entry_id])`` would leave those chunk rows orphaned
    and silently polluting search. We delete the literal row if present AND
    every row whose ``parent_entry_id`` matches, so both shapes are removed.
    """
    col = _get_collection()
    if not col:
        return _no_palace()
    ...
    try:
        ...
        # 1. Single-row entry stored under its literal id.
        single = col.get(ids=[entry_id], include=[])
        if single.get("ids"):
            col.delete(ids=[entry_id])
            deleted_ids.append(entry_id)

        # 2. Chunked entry: rows named {entry_id}_chunk_NNNNNN carrying
        #    parent_entry_id == entry_id. Delete the whole logical group.
        chunks = col.get(where={"parent_entry_id": entry_id}, include=[])
        ...
        # Audit parity with tool_delete_drawer (best-effort; _wal_log may be
        # absent on some builds — never let logging fail the delete).
        try:
            _wal_log("diary_delete", {"entry_id": entry_id, "deleted_ids": deleted_ids})
        except Exception:
            pass

    ...
    try:
        ...
        try:
            ...
            rows = conn.execute(
                ...
                FROM (
                    SELECT
                        COALESCE(wm.string_value, CAST(wm.int_value AS TEXT), '?') AS wing,
                        COALESCE(rm.string_value, CAST(rm.int_value AS TEXT), '?') AS room,
                        COALESCE(
                            pdm.string_value,
                            pem.string_value,
                            e.embedding_id
                    ...
                    LEFT JOIN embedding_metadata pdm
                        ON pdm.id = e.id AND pdm.key = 'parent_drawer_id'
                    LEFT JOIN embedding_metadata pem
                        ON pem.id = e.id AND pem.key = 'parent_entry_id'
                    WHERE c.name = ?
                    GROUP BY wing, room, logical_id
                ) logical_drawers
                GROUP BY wing, room
                """,
                (_config.collection_name,),
            ...
            conn.close()
        total = sum(int(count) for _wing, _room, count in rows)
        wing_rooms = {}
        for wing, room, count in rows:
            wing_rooms.setdefault(wing, {})[room] = int(count)
        counts = total, wing_rooms
    except Exception:
...
def tool_diary_read(agent_name: str, last_n: int = 10, wing: str = ""):
    ...
    try:
        ...
        for entry_id, doc, meta in zip(results["ids"], results["documents"], results["metadatas"]):
            ...
            entries.append(
                {
                    "entry_id": meta.get("parent_entry_id", entry_id),
                    "date": meta.get("date", ""),
...
def tool_diary_delete(entry_id: str):
    """Delete one logical diary entry, including every stored chunk."""
    global _metadata_cache
    ...
    try:
        ...
        documents = []
        metadatas = []
        direct = col.get(ids=[entry_id], include=["documents", "metadatas"])
        chunks = col.get(
            where={"parent_entry_id": entry_id},
            include=["documents", "metadatas"],
        )
        for result in (direct, chunks):
            ids = _chroma_field(result, "ids", []) or []
            docs = _chroma_field(result, "documents", []) or []
            metas = _chroma_field(result, "metadatas", []) or []
            for index, row_id in enumerate(ids):
                ...
                documents.append(docs[index] if index < len(docs) else "")
                metadatas.append(_safe_meta(metas[index] if index < len(metas) else {}))

        if not row_ids:
            return {"success": False, "error": f"Diary entry not found: {entry_id}"}
        if any(
            meta.get("type") != "diary_entry" and meta.get("room") != "diary" for meta in metadatas
        ):
            return {"success": False, "error": f"Not a diary entry: {entry_id}"}
        ...
        _wal_log(
            "diary_delete",
            {
                "entry_id": entry_id,
                "deleted_ids": row_ids,
                "deleted_meta": metadatas,
                "content_preview": "".join(documents)[:200],
            },
        )
        col.delete(ids=row_ids)
        _metadata_cache = None
        logger.info("Deleted diary entry: %s (%s rows)", entry_id, len(row_ids))
        return {
            "success": True,
            "entry_id": entry_id,
            "deleted_ids": row_ids,
            "chunks_deleted": len(row_ids),
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
TOOLS = {
    ...
    "mempalace_diary_delete": {
        "description": "Delete one logical diary entry by entry_id, including all chunks.",
        "input_schema": {
            "type": "object",
            "properties": {
                "entry_id": {
                    "type": "string",
                    "description": "Logical diary entry ID returned by diary_write or diary_read",
                },
            },
            "required": ["entry_id"],
        },

...
@pytest.mark.skip(reason="fork divergence: the tuned search pipeline is chroma-coupled by design (jor-shim, candidate_strategy=union accept-and-ignore; union not used via MCP). v3.4.1 pluggable-backend search (union via backend lexical_search + non-chroma vector_disabled guard) is not wired into the fork reranker. No impact on the chroma deployment. Re-enable if adopting backend-agnostic search.")
